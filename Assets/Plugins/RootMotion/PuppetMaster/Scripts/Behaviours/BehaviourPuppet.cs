...
namespace RootMotion.Dynamics {
	...
	public partial class BehaviourPuppet : BehaviourBase {
		...
		public struct MuscleProps {
			...
			[TooltipAttribute("How much will collisions with the muscle unpin it?")]
			/// <summary>
			/// How much will collisions with the muscle unpin it?
			/// </summary>
			[Range(0f, 1f)] public float unpinSelf;
			...
			[TooltipAttribute("How fast will muscles of this group regain their pin weight under invincibility (multiplier)?")]
			public float regainInvinciblePinSpeed;
		...
		[Tooltip("If this is enabled, the Behaviour will call OnBonesTooFar event instead of switching State when the character is about to be knocked out.")]
		/// <summary>
		/// If this is enabled, the Behaviour will call OnBonesTooFar event instead of switching State when the character is about to be knocked out.
		/// </summary>
		public bool useEventInsteadOfStateSwitching;
		...
		[Tooltip("Do not move position of puppet because of character collider while the beginning of getting up.")]
		/// <summary>
		/// Do not move position of puppet because of character collider while the beginning of getting up.
		/// </summary>
		public bool holdPositionWhileGetUp = true;

		[LargeHeader("Invincibility")]
		[Tooltip("Speed of constant regaining of pins")]
		public float invincibilityPinRegainSpeed = 10;
		[Tooltip("Max pin value multiplier, from 0 to 1")]
		public float invincibilityPinMax = 1f;
		[Tooltip("Speed of constant regaining of muscles")]
		public float invincibilityMuscleRegainSpeed = 1000;
		[Tooltip("Max muscle value multiplier, from 0 to 1")]
		public float invincibilityMuscleMax = 1f;

		[LargeHeader("Events")]

		[Tooltip("If useEventInsteadOfStateSwitching is enabled, is called instead switching State from Puppet to Unpinned.")]
		/// <summary>
		/// If useEventInsteadOfStateSwitching is enabled, is called instead switching State from Puppet to Unpinned.
		/// </summary>
		public UnityEvent onBonesTooFar;
		...
		[Tooltip("Called when the character switches from the Unpinned state.")]
		/// <summary>
		/// Called when the character switches from the Unpinned state.
		/// </summary>
		public PuppetEvent onLeaveUnpinned;
		...
		protected override void OnFixedUpdate(float deltaTime) {
			...
			if (state == State.Invincible)
			{
				foreach (Muscle m in puppetMaster.muscles)
				{
					if (!m.state.isDisconnected)
					{
						// Вот тут можно наверное будет поэкспериментировать с скоростями возврата вот этого всего
						//if (m.state.pinWeightMlp < invincibilityPinMax) m.state.pinWeightMlp += deltaTime * invincibilityPinRegainSpeed;
						//if (m.state.pinWeightMlp > invincibilityPinMax) m.state.pinWeightMlp = invincibilityPinMax;
						//m.state.pinWeightMlp = 1f;
						if (m.state.muscleWeightMlp < invincibilityMuscleMax) m.state.muscleWeightMlp += deltaTime * invincibilityMuscleRegainSpeed;
						if (m.state.muscleWeightMlp > invincibilityMuscleMax) m.state.muscleWeightMlp = invincibilityMuscleMax;
			...
			if (state != State.Unpinned && !puppetMaster.isKilling) {
				...
				foreach (Muscle m in puppetMaster.muscles) {
                    ...
                    if (hasCollidedSinceGetUp && !m.state.isDisconnected && !puppetMaster.isBlending && offset > 0f && pinW <= pinWeightThreshold && offset > props.knockOutDistance * knockOutDistanceSqr * stateMlp) {
						if (state != State.GetUp || getUpTargetFixed) {
							if (useEventInsteadOfStateSwitching)
							{
								onBonesTooFar.Invoke();
							}
							else
							{
								SetState(State.Unpinned);
                    ...
                    if (!m.state.isDisconnected)
                    {
                        ...
                        {
                            float speedF = 1f;
							float propSpeed = props.regainPinSpeed;

							if (state == State.GetUp) speedF = Mathf.Lerp(getUpRegainPinSpeedMlp, 1f, m.state.pinWeightMlp);
							if (state == State.Invincible) propSpeed = props.regainInvinciblePinSpeed;

                            m.state.pinWeightMlp += deltaTime * propSpeed * regainPinSpeed * speedF;
                        }
                    }
				}
				...
				foreach (Muscle m in puppetMaster.muscles) {
					if (m.props.group == Muscle.Group.LeftLeg || m.props.group == Muscle.Group.LeftFoot ||
						m.props.group == Muscle.Group.RightLeg || m.props.group == Muscle.Group.RightFoot) {
