...
namespace RootMotion.Dynamics
{
    ...
    public partial class BehaviourPuppet : BehaviourBase
    {
        ...
        {
            //Debug.Log($"SetState, from {state}, to {newState}");
            // If already in this state, do nothing
            if (state == newState) return;

            if (state == State.Unpinned) onLeaveUnpinned.Trigger(puppetMaster);
            ...
            switch (newState)
            {
                ...
                // Switching to the INVINCIBLE state
                case State.Invincible:
                    ...
                    foreach (Muscle m in puppetMaster.muscles)
                    {
                        if (!m.state.isDisconnected)
                        {
                            //m.state.pinWeightMlp = 1f; // RND-228, не влючать
                            //m.state.muscleWeightMlp = 1f;
                            m.state.muscleDamperAdd = 0f;
                            //m.positionOffset = Vector3.zero; // RND-228, не влючать
