...
#pragma warning restore IDE1005 // Вызов делегата можно упростить.
            ...
            if (LayerMaskExtensions.Contains(groundLayers, m.collision.gameObject.layer)) {
                ...
                if (puppetMaster.muscles[m.muscleIndex].props.group == Muscle.Group.LeftFoot ||
					puppetMaster.muscles[m.muscleIndex].props.group == Muscle.Group.RightFoot) return; // Do not damage if feet in contact with ground layers
            }
...
#pragma warning restore IDE1005 // Вызов делегата можно упростить.
		...
		public void UnPin(int muscleIndex, float unpin) {
			...
			for (int i = 0; i < puppetMaster.muscles.Length; i++) {
				UnPinMuscle(i, unpin * GetFalloff(i, muscleIndex, props.unpinParents, props.unpinChildren, props.unpinGroup, props.unpinSelf));
			}
